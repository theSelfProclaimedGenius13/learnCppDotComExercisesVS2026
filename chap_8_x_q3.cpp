...
int num_inputs_chap_8_x_q3() {
	int n;
	std::cin >> n;
	return n;
}

int yes_or_no_chap_8_x_q3() {
	char response;
	std::cin >> response;
...
void generate_random_number_chap_8_x_q3(int& min, int& max, int& chances) {
	...
	for(int i = 0; i < chances; ++i) {
		...
		int guess = num_inputs_chap_8_x_q3();
		if(guess == random_number) {
			...
			int try_again = yes_or_no_chap_8_x_q3();
			if(try_again == 0) {
...
void result_message_chap_8_x_q3() {
	std::cout << "Let's Play a Game of Lo! and Hi!\n";
	std::cout << "Enter the minimum Value:\n";
	static int min{ num_inputs_chap_8_x_q3() };
	std::cout << "Enter the maximum Value:\n";
	static int max{ num_inputs_chap_8_x_q3() };
	std::cout << "Enter the number of chances you want:\n";
	static int chances{ num_inputs_chap_8_x_q3() };
	generate_random_number_chap_8_x_q3(min, max, chances);
